...
class Gen1ScheduleEntry:
    ...
    @classmethod
    def from_bytes(cls, data: bytes) -> "Gen1ScheduleEntry":
...
@dataclass
class Gen1TimeOffset:
    point: Gen1OffsetPoint
    minutes: int

    @classmethod
    def from_bytes(cls, data: bytes) -> "Gen1TimeOffset":
...
@dataclass
class Gen1SunScheduleEntry:
    start: Gen1TimeOffset
    stop: Gen1TimeOffset
    weekdays: Gen1Weekday
    ...
    @classmethod
    def from_bytes(cls, data: bytes) -> "Gen1SunScheduleEntry":
        ...
        return cls(
            start=Gen1TimeOffset.from_bytes(data[0:2]),
            stop=Gen1TimeOffset.from_bytes(data[2:4]),
            weekdays=weekdays,
...
def parse_gen1_sun_schedule_config(config: bytes) -> list[Gen1SunScheduleEntry]:
