...
def _delete_event(device_id: str, resource_path: str) -> IngressMessageList:
    return IngressMessageList.model_validate_json(f"""
        [{{
            "entity": {{ "device": "{device_id}", "path": "{resource_path}" }},
            "metadata": {{ "sequence": 1, "source": "lemonbeatd" }},
            "op": "delete"
        }}]
        """)
...
async def test_gen1_returns_none_when_resource_missing(
    ...
    event = _delete_event(water_control.id, f"lemonbeat/0/{resource_name}")
    water_control.update_data(event.root[0])
...
async def test_gen1_schedule_config_and_count(water_control):
    ...
    assert water_control.schedule_count == len(config) // 7


@pytest.mark.asyncio
async def test_gen1_schedule_count_without_config(water_control):
    event = _delete_event(water_control.id, "lemonbeat/0/schedule_config")
    water_control.update_data(event.root[0])
...
async def test_gen1_button_config_time_missing(water_control):
    event = _delete_event(water_control.id, "lemonbeat/0/button_config_time")
    water_control.update_data(event.root[0])
...
async def test_gen1_get_watering_timer_missing(water_control):
    event = _delete_event(water_control.id, "lemonbeat/0/watering_timer_1")
    water_control.update_data(event.root[0])
...
async def test_gen1_water_control_temperature_missing(water_control):
    event = _delete_event(water_control.id, "lemonbeat/0/ambient_temperature")
    water_control.update_data(event.root[0])
